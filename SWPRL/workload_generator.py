...
class WorkloadGenerator(object):
    def __init__(
        self, config, workload_tables, random_seed, database_name, experiment_id=None
    ):
        ...
        self.workload_columns = [column for table in workload_tables for column in table.columns]
        ...
        if config["similar_workloads"] and config["unknown_queries"] == 0:
            ...
            embedder = WorkloadEmbedder(
                # Transform globally_partitionable_columns to list of lists.
                self.query_texts,
                0,
                embedder_connector,
                [list(map(lambda x: [x], self.globally_partitionable_columns))],
                retrieve_plans=True,
            )
    ...
    def _store_partionable_columns(self, query):
    ...
    def _workloads_from_tuples(self, tuples, unknown_query_probability=None):
        ...
        for tupl in tuples:
            ...
            for query_class, frequency in zip(query_classes, query_class_frequencies):
                ...
                self._store_partionable_columns(query)
    ...
    def _select_partitionable_columns(self):
        ...
        partitionable_columns = workload.partitionable_columns()
        selected_columns = []

        global_column_id = 0

        for table in self.workload_tables:
            selected_columns_table = []
            for column in table.columns:
                if column in partitionable_columns:
                    column.global_column_id = global_column_id
                    global_column_id += 1
                
                    selected_columns_table.append(column)
            selected_columns.append(selected_columns_table)
