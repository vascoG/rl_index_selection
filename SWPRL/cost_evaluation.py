...
from .what_if_index_creation import WhatIfPartitionCreation
...
class CostEvaluation:
    ...
    def _prepare_cost_calculation(self, indexes, store_size=False):
        for index in set(indexes) - self.current_partitions:
            self._simulate_or_create_index(index, store_size=store_size)
        for index in self.current_partitions - set(indexes):
            self._unsimulate_or_drop_index(index)

        assert self.current_partitions == set(indexes)
    ...
    def _simulate_or_create_index(self, index, store_size=False):
        ...
        self.current_partitions.add(index)
    ...
    def _unsimulate_or_drop_index(self, index):
        ...
        self.current_partitions.remove(index)
    ...
    def complete_cost_estimation(self):
        ...
        for index in self.current_partitions.copy():
            self._unsimulate_or_drop_index(index)
    ...
    def _request_cache(self, query, indexes):
        ...
        if q_i_hash in self.relevant_indexes_cache:
            relevant_partitions = self.relevant_indexes_cache[q_i_hash]
        else:
            relevant_partitions = self._relevant_partitions(query, indexes)
            self.relevant_indexes_cache[q_i_hash] = relevant_partitions
    ...
    @staticmethod
    def _relevant_partitions(query, indexes):
        relevant_partitions = [
